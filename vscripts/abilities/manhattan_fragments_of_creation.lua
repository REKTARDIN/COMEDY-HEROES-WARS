     ...
     if IsServer() then
          ...
          if #unit_table > 0 then
               for k, unit in pairs(unit_table) do
                    ...
                    ApplyDamage({attacker = self:GetCaster(), victim = unit, ability = self, damage = base_damage + (unit:GetHealth() * ptc), damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION  + DOTA_DAMAGE_FLAG_NO_DAMAGE_MULTIPLIERS, damage_type = DAMAGE_TYPE_MAGICAL, damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION})
               end
          end
