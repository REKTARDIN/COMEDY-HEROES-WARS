...
function manhattan_dark_projectile:OnSpellStart()
     if IsServer() then
          ...
          local units = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetCaster():GetOrigin(), self:GetCaster(), radius, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false )
          ...
          if #units > 0 then
               for _,unit in pairs(units) do
                    ...
                    total = total + (unit:GetAttackDamage() * damage)
               end
          end
          ...
          if #targets > 0 then
               for _,target in pairs(targets) do
                    ApplyDamage ( { attacker = self:GetCaster(), victim = target, ability = self, damage = total + self:GetAbilityDamage(), damage_type = DAMAGE_TYPE_MAGICAL, damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION  + DOTA_DAMAGE_FLAG_NO_DAMAGE_MULTIPLIERS})
               end
          end
