...
function Precache( context )
    ...
    PrecacheResource("soundfile", "soundevents/voscripts/game_sounds_vo_infinity.vsndevts", context)
    ...
    PrecacheResource("soundfile", "soundevents/custom_sounds_wheel.vsndevts", context)
    ...
    PrecacheResource("model", "models/items/courier/shibe_dog_cat/shibe_dog_cat_flying.vmdl", context)
...
DOTA_GOLD_PER_TICK = 3

GameMode.CAPTAINS_MODE_VOTES = {}
...
function GameMode:InitGameMode()
    ...
    GameRules:GetGameModeEntity():SetCustomScanCooldown(30)
    ...
    GameRules:GetGameModeEntity():SetPowerRuneSpawnInterval(180.0)
    GameRules:GetGameModeEntity():SetWeatherEffectsDisabled(false)
    GameRules:GetGameModeEntity():SetCustomTerrainWeatherEffect("maps/reef_assets/particles/reef_terrain.vpcf")

    CustomGameEventManager:RegisterListener("captain_mode_selected", Dynamic_Wrap(GameMode, 'OnCaptainModeSelected'))
    CustomNetTables:SetTableValue("globals", "game_settings", {is_cm_mode = 0, players = PlayerResource:GetPlayerCount()})
    ...
    if GameRules:State_Get() == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
        for i = 0, DOTA_MAX_PLAYERS - 1 do
            if PlayerResource:IsValidPlayerID(i) and PlayerResource:GetConnectionState(i) <= 2 then
                PlayerResource:ModifyGold(i, DOTA_GOLD_PER_TICK, false, DOTA_ModifyGold_Unspecified)
            end 
        end
    end 
end

function GameMode:OnCaptainModeSelected(data)
    GameMode.CAPTAINS_MODE_VOTES[data.PlayerID] = true

    CustomNetTables:SetTableValue("globals", "game_settings", {is_cm_mode = GameMode:IsCMMode(), players = PlayerResource:GetPlayerCount()})
end

function GameMode:IsCMMode()
    local count = 0 

    for player, state in pairs(GameMode.CAPTAINS_MODE_VOTES) do 
        count = count + 1
    end

    return (count == PlayerResource:GetPlayerCount()) and (GetMapName() == DOTA_MAP)
end
...
--[[
    {
   item_entindex_const             	= 769 (number)
   inventory_parent_entindex_const 	= 431 (number)
   suggested_slot                  	= -1 (number)
   item_parent_entindex_const      	= 431 (number)
}
]]
local sharedBfs = 
{
    "item_quilling_axe",
    "item_bloodletter",
    "item_ravager_of_fury"
}

function CDOTA_Item:IsCleaveItem()
    for _, item in pairs(sharedBfs) do 
        if item == self:GetAbilityName() then
            return true
        end
    end
...
function GameMode:InventoryFilter(ftable)
	--[[local target = EntIndexToHScript(ftable.inventory_parent_entindex_const)
    local item_p = EntIndexToHScript(ftable.item_entindex_const)

    if target and item_p:IsCleaveItem() then
        for _, item in pairs(sharedBfs) do 
            if target:HasItemInInventory(item) then

                if item_p then
                    UTIL_Remove(item_p)
                end

                CustomGameEventManager:Send_ServerToPlayer(target:GetPlayerOwner(), "create_error_message", {message = "You cant have more cleave items!"})
                return false
            end
        end
    end]]--
...
function GameMode:UpdateGameSettings()
    if GameMode:IsCMMode() then GameRules:SetPreGameTime( 8400.0 ) else GameRules:SetPreGameTime( 120.0 ) end
end


function GameMode:OnAllPlayersLoaded()
	if GameMode:IsCMMode() then CaptainsMode:Start() else Pick:Start() end
...
function GameMode:OnNPCSpawned(keys)
    ...
    if npc.bFirstSpawned == nil then
        ...
        ---- Тут хуй пойми что может произойти, лучше перестраховаться
        pcall(function() 
            Util:RegisterAbilities(npc)
        end)

        if npc:IsRealHero() then
            Util:OnHeroInGame(npc)
        else 
            if npc:IsIllusion() and npc:IsHero() and npc:GetPlayerOwner() ~= nil then
                Util:UpdateWearables(npc, npc:GetPlayerOwnerID())
            end
        end
    end
end
...
function GameMode:DmgFilter(ftable)
    if pcall(function ()
        ...
        Util:FilterDamage(victim, attacker, ftable)
    end) then else printd("We have an eror in dmg filter") print("We have an eror in dmg filter") end
...
function printd( msg )
	CustomGameEventManager:Send_ServerToAllClients("print_to_client", {data = msg})
end
...
function GameMode:OrderFilter(params)
    if params.units["0"] then
        local unit = EntIndexToHScript(params.units["0"])

        if IsValidEntity(unit) then
            if unit:HasModifier("modifier_cosmos_space_warp") or unit:HasModifier("modifier_anihilus_timeless_tenet") then
