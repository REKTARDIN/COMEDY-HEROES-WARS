...
LinkLuaModifier ("item_item_veil_of_discord_debuff", "items/item_veil_of_discord.lua", LUA_MODIFIER_MOTION_NONE)
     ...
     if IsServer() then
          ...
          if #units > 0 then
               for _,unit in pairs(units) do
                    unit:AddNewModifier( self:GetCaster(), self, "item_item_veil_of_discord_debuff", { duration = duration } )
               end
          end
...
if not item_item_veil_of_discord_debuff then item_item_veil_of_discord_debuff = class({}) end 

function item_item_veil_of_discord_debuff:IsHidden() return false end
function item_item_veil_of_discord_debuff:IsPurgable()  return true end
 
function item_item_veil_of_discord_debuff:DeclareFunctions ()
     local funcs = {
...
function item_item_veil_of_discord_debuff:GetModifierMagicalResistanceBonus (params)
